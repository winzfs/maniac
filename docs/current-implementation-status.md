...
- 선택된 카테고리 주요글 가로 스크롤
- 선택된 카테고리 전체보기 링크
- Category Boards 주요글 카드 클릭 시 게시글 상세 페이지로 이동
- CTA
...
- 홈 Category Boards의 카테고리 버튼은 페이지 이동 없이 홈 내부 주요글 목록만 변경한다.
- 홈 Category Boards의 게시글 카드는 해당 게시글 상세 페이지로 이동한다.
- 홈 Category Boards의 전체 카테고리 링크는 현재 선택된 카테고리 페이지로 이동한다.
...
- Category Boards의 주요글 카드는 게시글 상세로 이동한다.
...
- 게시글 상세는 모바일 1열, 데스크톱에서 본문/댓글 + 사이드 정보 카드 2단 구조
...
- PostCommentSection: 게시글 상세 댓글 작성 mock + 목록 UI
- PostCommentCard: 댓글 카드 UI
...
- `src/shared/data/mock-comments.ts`에서 mock comment 관리
- `src/features/home/CategoryPostScroller.tsx`에서 홈 Category Boards 주요글 스크롤 관리
- `src/features/boards/components/CategoryBoardPostFilter.tsx`에서 카테고리 상세 필터 UI 관리
- `src/features/boards/components/PostCommentSection.tsx`에서 게시글 댓글 섹션 관리
- `src/features/boards/components/PostCommentCard.tsx`에서 댓글 카드 관리
...
- 홈 Category Boards에서 카테고리 선택은 필터 역할만 하고, 게시글 카드 클릭은 상세 페이지 이동으로 유지한다.
...
따라서 다음 작업은 **features/boards 타입/schema/repository/action 뼈대 추가** 또는 **게시글 저장/sanitize/R2 업로드 연결 준비** 중 하나다.
