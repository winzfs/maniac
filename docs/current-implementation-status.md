...
Cloudflare Pages + Pages Functions + D1 기반으로 장비 기록, 이미지 업로드, 공개 장비 페이지, 커뮤니티 게시판, 외부 뉴스 캐시, 이메일/비밀번호 로그인, 내 정보/내 콘텐츠 관리, 홈 콘텐츠 피드, 관리자 화면, SEO 신뢰 페이지, 통합 검색, 카테고리 단위 글쓰기까지 1차 MVP가 구현되어 있다.
...
강한 SEO를 원하면 정적-safe 라우팅 또는 Pages Function HTML 메타 주입 전략이 필요하다.
```
...
게시판명, 카테고리명, 작성자명, slug 기반 broad matching은 제거했다.
...
공개/상세 장비 JSON API는 id를 먼저 조회하고, id 조회 실패 시 slug fallback 조회한다.
...
2. `/explore/[category]/`, `/explore/[category]/write/`, `/explore/post/?id=...`, `/search/` 집중 확인
3. 런타임 seed 정책 정리 및 관리자 seed/콘텐츠 관리 도구화
4. Search Console sitemap 상태 확인 및 주요 URL 색인 요청
5. 정식 도메인/문의 이메일 연결
6. 개인 페이지 noindex 정책 검토
7. 공개 장비/게시글 상세 SEO 라우팅 전략 검토
