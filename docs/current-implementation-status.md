...
### 3.4 게시판 상세 `/explore/[category]/[board]/`
...
### 3.5 글쓰기 `/explore/[category]/[board]/write/`
...
### 3.8 공개 장비 페이지 `/garage/ninja-400/`
...
- 상단은 공통 PageHeader로 breadcrumb와 메뉴 버튼을 일관되게 표시
...
- 상단 주황 라벨은 단순 Badge가 아니라 `MenuButton`으로 사용한다.
- 상세 페이지는 반드시 `Breadcrumbs`를 통해 현재 위치를 표시한다.
- breadcrumb 항목은 가능한 경우 `href`를 넣어 상위 페이지로 이동 가능하게 한다.
- 페이지 제목/설명/메뉴/breadcrumb는 `PageHeader`로 통합한다.
- 카테고리/게시판/글쓰기 페이지는 `PageHeader` 패턴을 우선 사용한다.
...
- MenuButton: 주황 메뉴 버튼 + 드롭다운 네비게이션
- Breadcrumbs: 현재 위치 표시 및 상위 이동
- PageHeader: breadcrumb/menu/title/description/action 통합
...
- `/explore/[category]/[board]/`까지 정적 게시글 목록 페이지 구현
- `/explore/[category]/[board]/write/` 글쓰기 mock 구현
- `features/editor`에 간단 HTML 에디터 경계 추가
...
카테고리 → 게시판 → 게시글 목록 → 글쓰기 mock 흐름 구현
내 정보 mock 페이지 구현
DB 스키마 1차 정교화 완료
D1 client boundary 추가
Equipment repository/action boundary 추가
실제 로그인/DB 연결/업로드 UI/게시글 저장은 아직 미구현
```

따라서 다음 작업은 **migration 검증 후 장비 CRUD UI** 또는 **게시글 상세/글쓰기 저장 mock** 중 하나다.
