...
### 3.6 공개 장비 페이지 `/garage/ninja-400/`
...
카테고리 → 게시판 → 게시글 목록 mock 흐름 구현
DB 스키마 1차 정교화 완료
D1 client boundary 추가
Equipment repository/action boundary 추가
실제 로그인/DB 연결/업로드 UI/게시글 작성은 아직 미구현
```

따라서 다음 작업은 **migration 검증 후 장비 CRUD UI** 또는 **게시글 상세/글쓰기 mock** 중 하나다.
