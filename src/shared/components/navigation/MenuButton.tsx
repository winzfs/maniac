...
export function MenuButton({ label = "메뉴" }: { label?: string }) {
  return (
    <details className="group relative inline-block">
      <summary className="inline-flex cursor-pointer list-none items-center gap-2 rounded-full bg-garage-orange px-3 py-1.5 text-xs font-bold text-white shadow-sm transition hover:opacity-90">
        <span>{label}</span>
        <span aria-hidden="true" className="transition group-open:rotate-180">⌄</span>
      </summary>
      <div className="absolute left-0 z-20 mt-2 w-72 overflow-hidden rounded-[1.5rem] border border-border bg-surface p-2 shadow-lg">
