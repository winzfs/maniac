...
export default function HomePage() {
  return <main className="container-shell space-y-12 py-5 sm:py-8 lg:space-y-16 lg:py-10">
    <header className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><h1 className="text-xl font-bold tracking-tight">Maniac Garage</h1><Link href="/garage/"><Button className="w-full sm:w-auto">내 차고 보기</Button></Link></header>
    ...
    <section className="grid gap-4 md:grid-cols-2"><Card><SectionHeader title="Maintenance Timeline Preview" /><p className="text-sm leading-6 text-text-secondary">엔진오일 교체 · 체인 점검 · 브레이크 패드 교체를 타임라인으로 정리합니다.</p></Card><Card><SectionHeader title="Popular Categories" /><div className="flex flex-wrap gap-2"><Badge label="바이크" tone="orange" /><Badge label="커스텀 PC" tone="muted" /><Badge label="카메라" tone="muted" /></div></Card></section>
