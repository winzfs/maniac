...
const popularCategories = equipmentCategories.slice(0, 5);
...
export default function HomePage() {
  return <main className="container-shell space-y-12 py-5 sm:py-8 lg:space-y-16 lg:py-10">
    ...
    <section className="grid gap-4 md:grid-cols-2"><Card><SectionHeader title="Maintenance Timeline Preview" /><p className="text-sm leading-6 text-text-secondary">엔진오일 교체 · 체인 점검 · 브레이크 패드 교체를 타임라인으로 정리합니다.</p></Card><Card><SectionHeader title="Popular Categories" action={<Link href="/explore/" className="text-xs font-semibold text-garage-orange">전체 보기</Link>} /><div className="flex flex-wrap gap-2">{popularCategories.map((category) => <Link key={category.slug} href={`/explore/${category.slug}/`}><Badge label={category.label} tone={category.slug === "motorcycle" ? "orange" : "muted"} /></Link>)}</div></Card></section>
