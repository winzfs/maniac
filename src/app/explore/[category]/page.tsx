...
export async function generateMetadata({ params }: { params: Promise<{ category: string }> }): Promise<Metadata> {
  ...
  const description = category
    ? `${category.description} ${category.label} 장비 자랑, 리뷰, 자유글, 질문 글을 둘러보세요.`
    : "장비덕후들의 기어 기록과 이야기를 둘러보세요.";
...
export default async function CategoryPage({ params }: { params: Promise<{ category: string }> }) {
  ...
  return (
    <main className="container-shell space-y-10 py-5 sm:py-8 lg:space-y-14">
      <PageHeader
        breadcrumbs={[{ label: "홈", href: "/" }, { label: category?.label ?? slug, href: `/explore/${slug}/` }]}
