    ...
    <main className="container-shell space-y-6 py-5 sm:py-8 lg:space-y-8">
      ...
      <section className="rounded-[2rem] border border-border bg-surface p-5 sm:p-6">
        ...
        <div className="mt-4 grid gap-3 sm:grid-cols-2">
          {parts.map((part) => (
            <article key={part.id} className="flex gap-3 rounded-3xl border border-border bg-white/60 p-3">
              {part.image_url ? (
                <img className="aspect-square h-20 w-20 shrink-0 rounded-2xl border border-border object-cover" src={part.image_url} alt={part.name} />
              ) : (
              ...
              <div className="min-w-0 flex-1">
                ...
                <h3 className="mt-1 truncate text-lg font-black">{part.brand ? `${part.brand} ` : ""}{part.name}</h3>
