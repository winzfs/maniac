...
function directGoogleNewsUrl(link: string) {
  return link.replace(/&amp;/g, "&");
}

function parseItems(xml: string, category: string, categoryLabel: string, maxItems: number): NewsItem[] {
  ...
  return itemMatches.slice(0, maxItems).map((itemXml, index) => {
    ...
    const link = directGoogleNewsUrl(tagValue(itemXml, "link"));
...
export const onRequestGet: PagesFunction = async ({ request }) => {
  ...
  const results = await Promise.allSettled(
    feeds.map(async (feed) => {
      ...
      return parseItems(xml, feed.category, feed.label, perFeed);
    }),
  );
