...
async function ensureImageSchema(db: D1Database) {
  ...
  await ignoreAlreadyExists(db.prepare("ALTER TABLE equipments ADD COLUMN main_image_asset_id TEXT REFERENCES image_assets(id) ON DELETE SET NULL").run());
}
...
export const onRequestPost: PagesFunction<Env> = async ({ request, env }) => {
  ...
  try {
    ...
    const objectKey = createImageObjectKey({
      userId: auth.user.id,
      purpose: "equipment",
      originalName: image.name || "equipment-image",
